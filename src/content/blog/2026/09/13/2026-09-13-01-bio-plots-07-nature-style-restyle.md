...
publishDate: 2026-09-13 08:00:00
tags: ["图表大全", "科研绘图", "技术选型"]
---
...
**第一，把全系列 190 张 PNG 按 Nature 图表规范全部重绘**（195 张图中 5 张是 3Dmol.js 交互 3D——139-142 与 188——本就没有 PNG）。数据一个字节没动，随机种子没动，图注里的每一个数字在重绘后依然与图内严格一致——变的只有像素。改造前的原图抽了 12 张存在 [`/img/bio-plots/before/`](/old/img/bio-plots/before/01-volcano.png) 里做对照，其余直接覆盖。
...
同一份数据、同一个脚本、同一行图注——只换了渲染层。前图存档于 [`before/`](/old/img/bio-plots/before/01-volcano.png)，点击可在新标签页放大对比。

### 图 1 火山图

![图 1 改造前](/old/img/bio-plots/before/01-volcano.png)

![图 1 改造后](/old/img/bio-plots/01-volcano.png)

红蓝换成 Okabe-Ito 的朱红/蓝（红绿色盲也能分），网格撤掉，字号缩一档。基因名锚线保留。

### 图 3 相关性热图

![图 3 改造前](/old/img/bio-plots/before/03-corr-heatmap.png)

![图 3 改造后](/old/img/bio-plots/03-corr-heatmap.png)

YlGnBu 色图未动，单元格数字改按填充亮度自动换黑/白——旧版深蓝格子上的深灰数字几乎不可读。

### 图 6 箱线图 / 图 7 小提琴图

![图 6 改造前](/old/img/bio-plots/before/06-boxplot.png)

![图 6 改造后](/old/img/bio-plots/06-boxplot.png)

![图 7 改造前](/old/img/bio-plots/before/07-violin.png)

![图 7 改造后](/old/img/bio-plots/07-violin.png)

### 图 10 UpSet 图

![图 10 改造前](/old/img/bio-plots/before/10-upset.png)

![图 10 改造后](/old/img/bio-plots/10-upset.png)

双面板自动钉上 a/b 字母——这是本次新增的导出级规则：任何多于一个数据面板的图，导出时按面板顺序自动标注粗体小写字母（colorbar 与孪生轴自动跳过）。

### 图 13 UMAP

![图 13 改造前](/old/img/bio-plots/before/13-umap.png)

![图 13 改造后](/old/img/bio-plots/13-umap.png)

### 图 51 Circos 风格共线性圈图

![图 51 改造前](/old/img/bio-plots/before/51-circos.png)

![图 51 改造后](/old/img/bio-plots/51-circos.png)

染色体弧与共线性带整体换 Okabe-Ito 低饱和版本，大圆图最怕的高饱和"儿童轮胎"感消失。

### 图 100 marker 点图 / 图 101 细胞通讯圈图

![图 100 改造前](/old/img/bio-plots/before/100-markers.png)

![图 100 改造后](/old/img/bio-plots/100-markers.png)

![图 101 改造前](/old/img/bio-plots/before/101-ccc-circle.png)

![图 101 改造后](/old/img/bio-plots/101-ccc-circle.png)

### 图 134 蛋白 Cα trace

![图 134 改造前](/old/img/bio-plots/before/134-ca-trace.png)

![图 134 改造后](/old/img/bio-plots/134-ca-trace.png)

### 图 174 真实数据 PCA（第六篇）

![图 174 改造前](/old/img/bio-plots/before/174_real_pca.png)

![图 174 改造后](/old/img/bio-plots/174_real_pca.png)

### 图 180 真实数据 Manhattan（第六篇）

![图 180 改造前](/old/img/bio-plots/before/180_real_manhattan.png)

![图 180 改造后](/old/img/bio-plots/180_real_manhattan.png)
...
<details>
<summary>展开查看：nature_style.py 完整代码</summary>
...
> **原文存档**：本文初版发布于旧站（Butterfly 主题）。如遇图表、代码高亮或 3D 交互显示异常，请查阅 [完整存档版](/old/2026/09/13/2026-09-13-01-生信图表大全第七篇-把190张图重画成Nature风格)。
