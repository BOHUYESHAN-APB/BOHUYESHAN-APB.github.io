...
<style>
  ...
  /* Version selector（不放在 banner，独立区块） */
  ...
  .r-ver-primary-title { font-weight: 700; color: #101a33; font-size: 1.02rem; }
  .r-ver-primary-desc { font-size: 0.83rem; color: #64748b; margin-top: 0.15rem; }
  ...
  .r-link-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 1rem; }
  ...
  .r-link-group ul { list-style: none; }
  ...
  /* Charts: 动态 SVG 量化图 */
  .r-chart-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1rem; }
  ...
  .r-chart-card .bar.d5 { animation-delay: 0.95s; }
