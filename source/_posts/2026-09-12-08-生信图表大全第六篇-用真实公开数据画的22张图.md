...
### 图 184 accession 表达 PCA：16 份在表达空间里分堆吗
...
**怎么读**：每点一个 accession（Rosette，14 基因面板），颜色按采集国家。与图 174 对照：基因型里西班牙和德国分居两端，表达里它们犬牙交错——**30% + 19% 的方差里，生长条件贡献大于地理**。10002（TueWal-2）在 PC1 上甩到最左：它是面板里 PR1 表达最高的一份（314723 RPK，第二名 1741 的 1.6 倍）——病原应激把它的表达谱整体推离了群体。

**用什么画**：16 个 accession × 14 基因 RPK 矩阵 → 按基因 z-score → SVD 取前两主成分。远程查询部分与图 183 同款，F+R 两链文件并行 6 线程。

**图注示例**：

> 图 184 accession 表达 PCA（真实数据，Rosette，16 份 × 14 基因）。PC1 30%、PC2 19%；分堆弱于基因型 PCA（图 174），生长条件是主要方差来源。
